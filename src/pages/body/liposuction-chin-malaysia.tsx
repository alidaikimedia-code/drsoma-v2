...
export default function ChinLiposuctionMalaysia() {
  ...
  const faqs = [
    { question: t('chinLipo.faq1Q'), answer: t('chinLipo.faq1A') },
    { question: t('chinLipo.faq2Q'), answer: t('chinLipo.faq2A') },
    { question: t('chinLipo.faq3Q'), answer: t('chinLipo.faq3A') },
    { question: t('chinLipo.faq4Q'), answer: t('chinLipo.faq4A') },
    { question: t('chinLipo.faq5Q'), answer: t('chinLipo.faq5A') },
    { question: t('chinLipo.faq6Q'), answer: t('chinLipo.faq6A') },
    { question: t('chinLipo.faq7Q'), answer: t('chinLipo.faq7A') },
    { question: t('chinLipo.faq8Q'), answer: t('chinLipo.faq8A') },
    { question: t('chinLipo.faq9Q'), answer: t('chinLipo.faq9A') },
    { question: t('chinLipo.faq10Q'), answer: t('chinLipo.faq10A') }
  ]

  const treatmentAreas = [
    { name: t('chinLipo.area1Name'), desc: t('chinLipo.area1Desc') },
    { name: t('chinLipo.area2Name'), desc: t('chinLipo.area2Desc') },
    { name: t('chinLipo.area3Name'), desc: t('chinLipo.area3Desc') }
  ]

  const benefits = [
    { title: t('chinLipo.benefit1Title'), desc: t('chinLipo.benefit1Desc'), icon: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" },
    { title: t('chinLipo.benefit2Title'), desc: t('chinLipo.benefit2Desc'), icon: "M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" },
    { title: t('chinLipo.benefit3Title'), desc: t('chinLipo.benefit3Desc'), icon: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" }
  ]

  const uspPoints = [
    { title: t('chinLipo.usp1Title'), desc: t('chinLipo.usp1Desc') },
    { title: t('chinLipo.usp2Title'), desc: t('chinLipo.usp2Desc') },
    { title: t('chinLipo.usp3Title'), desc: t('chinLipo.usp3Desc') },
    { title: t('chinLipo.usp4Title'), desc: t('chinLipo.usp4Desc') }
  ]

  const processSteps = [
    { num: "1", title: t('chinLipo.step1'), desc: t('chinLipo.step1Desc') },
    { num: "2", title: t('chinLipo.step2'), desc: t('chinLipo.step2Desc') }
  ]

  const resultsTimeline = [
    { time: t('chinLipo.resTime1'), desc: t('chinLipo.resDesc1') },
    { time: t('chinLipo.resTime2'), desc: t('chinLipo.resDesc2') },
    { time: t('chinLipo.resTime3'), desc: t('chinLipo.resDesc3') },
    { time: t('chinLipo.resTime4'), desc: t('chinLipo.resDesc4') }
  ]

  const testimonials = [
    { text: t('chinLipo.test1Text'), name: t('chinLipo.test1Name') },
    { text: t('chinLipo.test2Text'), name: t('chinLipo.test2Name') },
    { text: t('chinLipo.test3Text'), name: t('chinLipo.test3Name') }
  ]
    ...
    <>
      ...
      <main className="chin-lipo-page bg-white text-zinc-900">
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            <div className="order-1 lg:order-1">
              ...
              <h1 className="text-4xl md:text-6xl font-semibold tracking-tighter text-zinc-900 leading-[1.05] mb-3 md:mb-6 animate-fadeInUp opacity-0" style={{animationDelay: '0.2s', animationFillMode: 'forwards'}}>
                <span className="hover:text-[#FE7623] transition-colors duration-300 cursor-default">{t('chinLipo.heroTitle')}</span> <br />
                <span className="text-[#FE7623] text-[20px] md:text-[30px] tracking-[1px] transition-colors duration-300 cursor-default mt-[10px] block">{t('chinLipo.heroSubtitle')}</span>
              </h1>
              <div className="text-zinc-600 leading-relaxed mb-4 md:mb-8 max-w-xl animate-fadeInUp opacity-0 text-justify" style={{animationDelay: '0.3s', animationFillMode: 'forwards'}}>
                <p style={{fontSize: '14px'}}>{t('chinLipo.heroDesc')}</p>
              </div>
              ...
              <div className="flex items-center gap-4 mt-8 animate-fadeInUp opacity-0" style={{animationDelay: '0.5s', animationFillMode: 'forwards'}}>
                ...
                <span className="text-sm text-zinc-600 font-medium">{t('chinLipo.trustedBy')} <span className="text-[#FE7623] font-bold">500+</span> {t('chinLipo.happyPatients')}</span>
              </div>
            </div>
              ...
              <div className="hidden md:block absolute bottom-4 left-4 right-4 md:bottom-6 md:left-6 md:right-6">
                <div className="bg-white backdrop-blur-md p-3 md:p-5 rounded-xl border border-zinc-100 shadow-xl group-hover:translate-y-[-5px] transition-transform duration-500">
                  <div className="flex items-center gap-3 md:gap-4">
                    ...
                    <div>
                      <p className="text-[10px] md:text-xs font-bold uppercase tracking-wide text-zinc-400">{t('chinLipo.goal')}</p>
                      <p className="text-sm md:text-base font-semibold text-zinc-900">{t('chinLipo.goalName')}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-12 items-start">
              ...
              <div data-animate="def-2" className={`md:col-span-7 transition-all duration-700 ${isVisible('def-2') ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
                <h3 className="text-sm font-bold text-[#FE7623] uppercase tracking-wide mb-6">{t('chinLipo.areasTitle')}</h3>
        ...
        <section className="py-8 md:py-24 px-6 bg-zinc-50">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            <div data-animate="method" className={`transition-all duration-700 ${isVisible('method') ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
              <h2 className="text-2xl font-semibold tracking-tight text-[#FE7623] mb-6">{t('chinLipo.methodTitle')}</h2>
              <p className="text-sm text-zinc-600 leading-relaxed mb-8">
                {t('chinLipo.methodDesc')}
              </p>
              ...
              <h3 className="text-xl font-semibold tracking-tight text-[#FE7623] mb-6">{t('chinLipo.resultsTitle')}</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div data-animate="img-1" className={`relative rounded-2xl overflow-hidden shadow-2xl h-[600px] lg:sticky lg:top-8 transition-all duration-700 group ${isVisible('img-1') ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
              ...
              <div className="absolute bottom-8 left-8 right-8 bg-white p-6 rounded-xl border border-zinc-200 shadow-lg">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-black">{t('chinLipo.lastingTitle')}</h4>
                    <p className="text-xs text-black mt-1 leading-relaxed">
                      {t('chinLipo.lastingDesc')}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <section className="py-8 md:py-24 px-6 bg-white">
          <div className="max-w-7xl mx-auto">
            <h2 data-animate="benefits-header" className={`text-2xl font-semibold tracking-tight text-center text-[#FE7623] mb-12 transition-all duration-700 ${isVisible('benefits-header') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>{t('chinLipo.benefitsTitle')}</h2>
        ...
        <section className="py-8 md:py-24 px-6 bg-zinc-900 text-white overflow-hidden">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div data-animate="img-2" className={`order-2 lg:order-1 relative rounded-2xl overflow-hidden shadow-2xl h-[500px] border border-zinc-800 transition-all duration-700 ${isVisible('img-2') ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
              ...
              <div className="absolute bottom-8 left-8">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#FE7623]/20 border border-[#FE7623]/50 text-[#FE7623] text-[10px] font-medium uppercase tracking-widest mb-2">
                  {t('chinLipo.safetyFirst')}
                </div>
                <h3 className="text-lg font-medium">{t('chinLipo.hospitalGrade')}</h3>
                <p className="text-sm text-white mt-2 max-w-xs">{t('chinLipo.hospitalGradeDesc')}</p>
              </div>
            </div>
            ...
            <div data-animate="usp" className={`order-1 lg:order-2 transition-all duration-700 ${isVisible('usp') ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
              <h2 className="text-2xl font-semibold tracking-tight text-white mb-8">{t('chinLipo.uspTitle')}</h2>
          ...
          <div className="flex containers w-full">
            <div className="flex flex-col w-full">
              <div data-animate="comparison" className={`w-full transition-all duration-700 ${isVisible('comparison') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                <h2 className="text-2xl font-semibold mb-10 text-center text-[#FE7623]">{t('chinLipo.compareTitle')}</h2>
                ...
                <div className="w-full border border-zinc-200 rounded-xl overflow-hidden bg-white shadow-sm hover:shadow-lg transition-shadow duration-300">
                  ...
                  <div className="grid grid-cols-3 bg-zinc-50 px-6 py-6 border-b border-zinc-200 text-xs font-bold uppercase tracking-wider text-para-black">
                    <div>{t('chinLipo.feature')}</div>
                    <div className="text-[#FE7623]">{t('chinLipo.compChin')}</div>
                    <div>{t('chinLipo.compNonSurg')}</div>
                  </div>

                  {/* Rows */}
                  {[
                    { feat: t('chinLipo.fatRemoval'), lipo: t('chinLipo.chinFatRemoval'), nonsurg: t('chinLipo.nonSurgFatRemoval'), lipoHighlight: false },
                    { feat: t('chinLipo.results'), lipo: t('chinLipo.chinResults'), nonsurg: t('chinLipo.nonSurgResults'), lipoHighlight: false },
                    { feat: t('chinLipo.bestFor'), lipo: t('chinLipo.chinBestFor'), nonsurg: t('chinLipo.nonSurgBestFor'), lipoHighlight: false }
                  ].map((row, i) => (
                ...
                <p className="text-xs text-center text-zinc-400 mt-6 max-w-2xl mx-auto">{t('chinLipo.compareNote')}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-8 md:py-24 px-6 bg-zinc-50">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            <div data-animate="candidates" className={`transition-all duration-700 ${isVisible('candidates') ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
              <h2 className="text-2xl font-semibold tracking-tight text-[#FE7623] mb-8">{t('chinLipo.candidateTitle')}</h2>
              <div className="bg-white rounded-2xl p-8 border border-zinc-200 shadow-sm">
                <h3 className="text-lg font-bold text-zinc-900 mb-6">{t('chinLipo.idealTitle')}</h3>
                <ul className="space-y-4 mb-8">
                  {[t('chinLipo.ideal1'), t('chinLipo.ideal2'), t('chinLipo.ideal3'), t('chinLipo.ideal4')].map((item, i) => (
                ...
                <div className="border-t border-zinc-200 pt-6">
                  <h4 className="text-lg font-bold text-zinc-900 mb-4">{t('chinLipo.notSuitableTitle')}</h4>
                  <ul className="space-y-3">
                    {[t('chinLipo.notSuitable1'), t('chinLipo.notSuitable2')].map((item, i) => (
            ...
            <div data-animate="recovery" className={`transition-all duration-700 ${isVisible('recovery') ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
              <h2 className="text-2xl font-semibold tracking-tight text-[#FE7623] mb-8">{t('chinLipo.recoveryTitle')}</h2>
              <div className="relative space-y-8 pl-8 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-px before:bg-zinc-200">
                {[
                  { time: t('chinLipo.recTime1'), desc: t('chinLipo.recDesc1'), active: true },
                  { time: t('chinLipo.recTime2'), desc: t('chinLipo.recDesc2'), active: false },
                  { time: t('chinLipo.recTime3'), desc: t('chinLipo.recDesc3'), active: false },
                  { time: t('chinLipo.recTime4'), desc: t('chinLipo.recDesc4'), active: false }
                ].map((step, i) => (
        ...
        <section className="py-8 md:py-24 px-6 bg-white border-t border-zinc-200">
          <div className="max-w-7xl mx-auto">
            <h2 data-animate="testimonial-header" className={`text-2xl font-semibold text-[#FE7623] text-center mb-12 transition-all duration-700 ${isVisible('testimonial-header') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>{t('chinLipo.storiesTitle')}</h2>
        ...
        <section className="py-8 md:py-24 px-6 bg-zinc-50">
          <div className="max-w-3xl mx-auto text-center">
            <h2 data-animate="pricing-header" className={`text-2xl font-semibold text-[#FE7623] mb-6 transition-all duration-700 ${isVisible('pricing-header') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>{t('chinLipo.pricingTitle')}</h2>
            <p className="text-sm text-zinc-500 mb-8">{t('chinLipo.pricingDesc')}</p>
            <div className="bg-zinc-50 border border-zinc-200 rounded-xl p-8 text-left">
              <h4 className="text-sm font-semibold text-zinc-900 mb-4">{t('chinLipo.costFactorsTitle')}</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[t('chinLipo.priceFactor1'), t('chinLipo.priceFactor2'), t('chinLipo.priceFactor3'), t('chinLipo.priceFactor4')].map((item, i) => (
              ...
              <div className="mt-8 pt-6 border-t border-zinc-200 text-center">
                <p className="text-sm font-medium text-zinc-900">{t('chinLipo.priceNote')}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-12 md:py-24 px-6 bg-gradient-to-b from-zinc-50 to-white border-t border-zinc-200">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-row items-start gap-3 mb-10">
              ...
              <h3 className="text-sm font-bold text-[#FE7623] uppercase tracking-widest flex-1">{t('chinLipo.refTitle')}</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { name: t('chinLipo.harvardName'), url: "https://www.health.harvard.edu/", desc: t('chinLipo.harvardDesc'), color: "from-red-500 to-red-600" },
                { name: t('chinLipo.mayoName'), url: "https://www.mayoclinic.org/", desc: t('chinLipo.mayoDesc'), color: "from-blue-500 to-blue-600" },
                { name: t('chinLipo.pubmedName'), url: "https://pubmed.ncbi.nlm.nih.gov/", desc: t('chinLipo.pubmedDesc'), color: "from-green-500 to-green-600" }
              ].map((source, i) => (
        ...
        <section className="pt-0 pb-8 md:pt-0 md:pb-24 px-6 bg-white">
          <div className="max-w-3xl mx-auto">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h2 className="text-2xl font-semibold text-[#FE7623]">{t('chinLipo.faqTitle')}</h2>
                <p className="text-sm text-zinc-500">{t('chinLipo.faqDesc')}</p>
              </div>
            </div>
        ...
        <section className="pt-0 pb-8 md:pt-0 md:pb-24 px-6 bg-white relative overflow-hidden">
          ...
          <div data-animate="cta" className={`max-w-3xl mx-auto text-center relative z-10 transition-all duration-700 ${isVisible('cta') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <h2 className="text-2xl font-semibold tracking-tight text-[#FE7623] mb-6 transition-colors duration-300 cursor-default">{t('chinLipo.ctaTitle')}</h2>
