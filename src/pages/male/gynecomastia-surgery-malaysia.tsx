...
export default function GynecomastiaSurgeryMalaysia() {
  ...
  const faqs = Array.from({ length: 7 }, (_, i) => ({
    question: t(`gyn.faq${i}Q`),
    answer: t(`gyn.faq${i}A`),
  }))

  const surgicalTechniques = Array.from({ length: 3 }, (_, i) => ({
    title: t(`gyn.tech${i}Title`),
    desc: t(`gyn.tech${i}Desc`),
  }))

  const gynecomastiaCauses = Array.from({ length: 4 }, (_, i) => t(`gyn.cause${i}`))

  const benefits = Array.from({ length: 5 }, (_, i) => t(`gyn.benefit${i}`))

  const candidateCriteria = Array.from({ length: 5 }, (_, i) => t(`gyn.cand${i}`))

  const comparisonData = Array.from({ length: 4 }, (_, i) => ({
    aspect: t(`gyn.comp${i}Aspect`),
    surgery: t(`gyn.comp${i}Surgery`),
    exercise: t(`gyn.comp${i}Exercise`),
  }))
    ...
    <>
      <Head>
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "@type": "FAQPage",
              "@id": "https://drsoma-v2.vercel.app/male/gynecomastia-surgery-malaysia/#faq",
              "mainEntity": [
                {
                  "@type": "Question",
                  "name": "What is gynecomastia surgery?",
                  "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "It is a surgical procedure that removes excess male breast tissue to create a flatter chest."
                  }
                },
                {
                  "@type": "Question",
                  "name": "Is gynecomastia surgery permanent?",
                  "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "Yes, removed glandular tissue does not regrow under normal conditions."
                  }
                },
                {
                  "@type": "Question",
                  ...
                  "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "Scars are minimal and typically fade well over time."
                  }
                },
                {
                  "@type": "Question",
                  ...
                  "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "Discomfort is usually mild to moderate and manageable with medication."
                  }
                },
                {
                  "@type": "Question",
                  "name": "How long is recovery after surgery?",
                  "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "Most patients return to normal activities within 1–2 weeks."
                  }
                },
                {
                  "@type": "Question",
                  "name": "Can gynecomastia return after surgery?",
                  "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "Recurrence is rare unless caused by new hormonal imbalance or medication."
                  }
                },
                {
                  "@type": "Question",
                  "name": "Is gynecomastia surgery safe in Malaysia?",
                  "acceptedAnswer": {
                    "@type": "Answer",
                    "text": "Yes, when performed by a qualified plastic surgeon in a licensed facility under the Ministry of Health Malaysia, the procedure is safe and follows international standards."
                  }
                }
              ]
            })
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              "@context": "https://schema.org",
              "@type": "Service",
              "@id": "https://drsoma-v2.vercel.app/male/gynecomastia-surgery-malaysia/#service",
              "name": "Gynecomastia Surgery Malaysia | Male Breast Reduction – Dr. Soma",
              "url": "https://drsoma-v2.vercel.app/male/gynecomastia-surgery-malaysia/",
              "description": "Gynecomastia surgery in Malaysia by Dr. Soma. Male breast reduction for a firmer, masculine chest. Expert surgical treatment. Book consultation today.",
              "aggregateRating": {
        ...
        <style dangerouslySetInnerHTML={{ __html: `
          ...
          .gynecomastia-page ::selection { background-color: #FE7623; color: white; }
      ...
      <main className="gynecomastia-page bg-white text-zinc-900 w-full max-w-full">
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            <div className="order-1 lg:order-1">
              ...
              <h1 className="text-4xl md:text-6xl font-semibold tracking-tighter text-zinc-900 leading-[1.05] mb-3 md:mb-6 animate-fadeInUp opacity-0" style={{animationDelay: '0.2s', animationFillMode: 'forwards'}}>
                <span className="hover:text-[#FE7623] transition-colors duration-300 cursor-default">{t('gyn.title1')}</span> <br />
                <span className="text-[#FE7623] text-[18px] md:text-[28px] tracking-[1px] transition-colors duration-300 cursor-default mt-[10px] block">{t('gyn.subtitle')}</span>
              </h1>
              <div className="text-zinc-600 leading-relaxed mb-4 md:mb-8 max-w-xl animate-fadeInUp opacity-0 text-justify" style={{animationDelay: '0.3s', animationFillMode: 'forwards'}}>
                <p style={{fontSize: '14px'}}>{t('gyn.heroDesc1')}</p>
                <p style={{fontSize: '14px', marginTop: '12px'}}>{t('gyn.heroDesc2')}</p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-4 mt-8 animate-fadeInUp opacity-0" style={{animationDelay: '0.5s', animationFillMode: 'forwards'}}>
                ...
                <span className="text-sm text-zinc-600 font-medium text-center sm:text-left">{t('gyn.trustedBy')} <span className="text-[#FE7623] font-bold">500+</span> {t('gyn.happyPatients')}</span>
              </div>
            </div>
              ...
              <div className="hidden md:block absolute bottom-4 left-4 right-4 md:bottom-6 md:left-6 md:right-6">
                <div className="bg-white backdrop-blur-md p-3 md:p-5 rounded-xl border border-zinc-100 shadow-xl group-hover:translate-y-[-5px] transition-transform duration-500">
                  <div className="flex items-center gap-3 md:gap-4">
                    ...
                    <div>
                      <p className="text-[10px] md:text-xs font-bold uppercase tracking-wide text-zinc-400">{t('gyn.procedureFocus')}</p>
                      <p className="text-sm md:text-base font-semibold text-zinc-900">{t('gyn.maleChestContouring')}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-12 px-4 md:px-8">
              <div data-animate="def-1" className={`md:col-span-6 transition-all duration-700 ${isVisible('def-1') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                <h2 className="text-3xl font-semibold tracking-tight text-[#FE7623] mb-6">{t('gyn.whatIsTitle')}</h2>
              ...
              <div data-animate="def-2" className={`md:col-span-6 transition-all duration-700 ${isVisible('def-2') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                <h3 className="text-sm font-bold text-[#FE7623] uppercase tracking-wide mb-6">{t('gyn.howWorksTitle')}</h3>
                ...
                <div className="p-4 bg-orange-50 rounded-xl border border-orange-100 mt-6">
                  <p className="text-sm text-zinc-700"><strong>Important:</strong> {t('gyn.importantNote')}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="py-8 md:py-24 px-6 bg-zinc-50 overflow-hidden">
          <div className="max-w-7xl mx-auto">
            <div data-animate="recovery-header" className={`max-w-2xl mb-12 px-4 md:px-8 transition-all duration-700 ${isVisible('recovery-header') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
              <h2 className="text-3xl font-semibold tracking-tight text-[#FE7623] mb-4">{t('gyn.recoveryTitle')}</h2>
              <p className="text-zinc-500 text-sm">{t('gyn.recoveryDesc')}</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4 md:px-8">
              {Array.from({ length: 5 }, (_, i) => ({
                time: t(`gyn.rec${i}Time`),
                desc: t(`gyn.rec${i}Desc`),
              })).map((item, i) => (
            ...
            <div className="px-4 md:px-8 mt-8">
              <div className="p-4 bg-blue-50 rounded-xl border border-blue-100">
                <p className="text-sm text-blue-700"><strong>Note:</strong> {t('gyn.recoveryNote')}</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-8 md:py-24 px-6 bg-white overflow-hidden">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center px-4 md:px-8">
            ...
            <div data-animate="benefits-content" className={`order-1 lg:order-2 transition-all duration-700 ${isVisible('benefits-content') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
              <h2 className="text-3xl font-semibold tracking-tight text-[#FE7623] mb-6">{t('gyn.benefitsTitle')}</h2>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center px-4 md:px-8">
              <div data-animate="candidate-content" className={`transition-all duration-700 ${isVisible('candidate-content') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                <h2 className="text-3xl font-semibold tracking-tight text-[#FE7623] mb-6">{t('gyn.candidateTitle')}</h2>
                <p className="text-zinc-500 text-sm leading-relaxed mb-6">
                  {t('gyn.candidateDesc')}
                </p>
                <div className="space-y-3">
              ...
              <div data-animate="additional-content" className={`transition-all duration-700 ${isVisible('additional-content') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                <h3 className="text-sm font-bold text-[#FE7623] uppercase tracking-wide mb-6">{t('gyn.distinctionTitle')}</h3>
                <p className="text-zinc-500 text-sm leading-relaxed mb-6">
                  {t('gyn.distinctionDesc')}
                </p>
                <div className="p-4 bg-amber-50 rounded-xl border border-amber-100">
                  <p className="text-sm text-amber-800"><strong>Note:</strong> {t('gyn.exerciseNote')}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="py-8 md:py-24 px-4 md:px-8 lg:px-16 bg-white overflow-hidden">
          <div className="w-full max-w-[1400px] mx-auto">
            <div data-animate="comparison-header" className={`text-center mb-12 transition-all duration-700 ${isVisible('comparison-header') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
              <h2 className="text-3xl font-semibold tracking-tight text-[#FE7623] mb-4">{t('gyn.compTitle')}</h2>
              <p className="text-zinc-500 text-sm max-w-2xl mx-auto">{t('gyn.compDesc')}</p>
            </div>
              ...
              <table className="w-full text-sm bg-zinc-50 rounded-2xl overflow-hidden shadow-lg">
                <thead>
                  <tr className="border-b border-zinc-200 bg-zinc-100">
                    <th className="text-left py-4 px-4 text-zinc-600 font-semibold">{t('gyn.compAspect')}</th>
                    <th className="text-left py-4 px-4 text-[#FE7623] font-semibold">{t('gyn.compSurgery')}</th>
                    <th className="text-left py-4 px-4 text-zinc-600 font-semibold">{t('gyn.compExercise')}</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12 px-4 md:px-8">
            ...
            <div className="lg:col-span-1">
              <div data-animate="pricing" className={`bg-gradient-to-br from-zinc-900 to-zinc-800 text-white p-8 rounded-2xl lg:sticky lg:top-24 shadow-2xl transition-all duration-700 hover:shadow-[0_25px_60px_-15px_rgba(254,118,35,0.3)] group card-shine ${isVisible('pricing') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                ...
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-xl font-semibold group-hover:text-[#FE7623] transition-colors duration-300">{t('gyn.pricingTitle')}</h3>
                </div>
                <p className="text-zinc-400 text-sm mb-6">{t('gyn.pricingDesc')}</p>
                <ul className="space-y-4 mb-8">
                  {Array.from({ length: 5 }, (_, i) => t(`gyn.price${i}`)).map((item, i) => (
            ...
            <div className="lg:col-span-2">
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  <h2 className="text-2xl font-semibold text-[#FE7623]">{t('gyn.faqTitle')}</h2>
                  <p className="text-sm text-zinc-500">{t('gyn.faqDesc')}</p>
                </div>
              </div>
              <div className="space-y-3">
                {faqs.map((faq, index) => (
        ...
        <section className="py-8 md:py-24 px-6 bg-white relative overflow-hidden w-full">
          ...
          <div data-animate="cta" className={`max-w-5xl mx-auto text-center relative z-10 transition-all duration-700 ${isVisible('cta') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-semibold tracking-tight text-[#FE7623] mb-6 transition-colors duration-300 cursor-default px-4">{t('gyn.ctaTitle')}</h2>
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3 px-4">
              {Array.from({ length: 5 }, (_, i) => t(`gyn.seoTag${i}`)).map((tag, i) => (
